    ...
    public function refunds_update()
    {
        ...
        if($orderRow && Order_Class::isRefundmentApply($orderRow))
        {
        	...
        	if($goodsOrderRow && $goodsOrderRow['is_send'] != 2)
        	{
        		...
        		if($billRow)
        		{
        			$message = '此订单金额已被商家结算完毕，请直接与商家联系';
			        $this->redirect('refunds',false);
			        Util::showMessage($message);
        		}
    	...
    	else
    	{
			...
			else
			{
		    	...
		    	else
		    	{
		    		...
		    		$orderRow = $orderObj->getObj('order_no  = "'.IFilter::act($return['order_no']).'" and pay_status = 0 and status = 1 and user_id = '.$user_id);
		    		if(empty($orderRow))
		    		{
		    		...
		    		$order_id   = Order_Class::updateOrderStatus($return['order_no']);
		    		$is_success = false;
		    		if($order_id)
		    		{
						//扣除余额并且记录日志

...
class System extends IController
{
    ...
    */
	...
	{
		$result     = false;
		$clientList = IClient::supportClient(); //支持的客户端平台
		$applyArray = array(); //要写入config的数据
		foreach($clientList as $key => $client)
		{
			//过滤不启用的主题
			$clientThemeList = array_filter( IFilter::act(IReq::get($client)) );
			if($clientThemeList)
			{
				$applyArray[$client] = $clientThemeList;
			}
		}

		//检查PC端必须分配管理员后台的主题
		if($applyArray && $applyArray["pc"])
		{
			foreach($applyArray["pc"] as $theme => $skin)
			{
				if(common::themeType($theme) == "system")
				{
					$result = true;
				}
			}
		...
		if($result)
		{
			IWeb::$app->config['theme'] = $applyArray;
			$message = "修改成功";
			Config::edit('config/config.php',array("theme" => $applyArray));
		}
		else
		{
			$message = "后台管理必须要在PC端应用";
		}

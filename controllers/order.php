...
class Order extends IController
{
	...
	public function order_refundment_doc()
	{
		...
		$isNotBalance = IFilter::act(IReq::get('isNotBalance'),'int');
		...
		$tb_refundment_doc = new IModel('refundment_doc');
		if($refunds_id && !$order_goods_id)
		{
			$refundmentRow  = $tb_refundment_doc->getObj('id = '.$refunds_id);
			$tempOrderGoods = $orderGoodsDB->getObj("order_id = ".$refundmentRow['order_id']." and goods_id = ".$refundmentRow['goods_id']." and product_id = ".$refundmentRow['product_id']);
			$order_goods_id = $tempOrderGoods['id'];
		}
		...
		if($amount > $actAmount)
		{
			die('<script text="text/javascript">parent.actionCallback("填写的退款金额不能大于实际用户支付的金额");</script>');
		}
		...
		if(!$refunds_id)
		{
			...
			if(!$orderGoodsRow)
			{
				die('<script text="text/javascript">parent.actionCallback("要退款的商品不存在");</script>');
			}
		...
		$result = Order_Class::refund($refunds_id,$this->admin['admin_id'],'admin',$isNotBalance);
		if($result)
		{
			//记录操作日志
			$logObj = new log('db');

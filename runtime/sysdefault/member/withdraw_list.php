	...
	<form action="<?php echo IUrl::creatUrl("/member/withdraw_update/type/del");?>" method="post">
		<table class="list_table">
			...
			<tbody>
				...
				<?php $query = new IQuery("withdraw as w");$query->join = "left join member as m on w.user_id = m.user_id left join user as u on u.id = w.user_id";$query->where = "is_del = 0";$query->page = "$page";$query->order = "id desc";$query->fields = "w.*,u.username,m.balance";$items = $query->find(); foreach($items as $key => $item){?>
				<tr>
					...
					<td>
						<a href='<?php echo IUrl::creatUrl("/member/withdraw_detail/id/".$item['id']."");?>'><img class="operator" src="<?php echo $this->getWebSkinPath()."images/admin/icon_edit.gif";?>" alt="查看" title="查看" /></a>
						<a href='javascript:void(0)' onclick="delModel({link:'<?php echo IUrl::creatUrl("/member/withdraw_update/type/del/id/".$item['id']."");?>',msg:'确定要把此信息放入到回收站么？'});"><img class="operator" src="<?php echo $this->getWebSkinPath()."images/admin/icon_del.gif";?>" alt="删除" title="删除" /></a>
					</td>
				</tr>
				<?php }?>
			</tbody>

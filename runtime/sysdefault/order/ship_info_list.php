...
<body>
	...
	<div class="operating">
		...
		<a href="javascript:;"><button class="operating_btn" type="button" onclick="location.href='<?php echo IUrl::creatUrl("/order/ship_recycle_list");?>'"><span class="recycle">回收站</span></button></a>
	</div>
	<div class="field">
	...
	<form action="<?php echo IUrl::creatUrl("/order/ship_info_del");?>" method="post" name="ship_list">
		<table class="list_table">
			...
			<tbody>
				...
				<?php $query = new IQuery("merch_ship_info");$query->page = "$page";$query->where = "is_del = 1 and seller_id = 0";$query->order = "id desc";$items = $query->find(); foreach($items as $key => $item){?>
				<tr>
					...
					<td>
						<a href="<?php echo IUrl::creatUrl("/order/ship_info_edit/sid/".$item['id']."");?>"><img class="operator" src="<?php echo $this->getWebSkinPath()."images/admin/icon_edit.gif";?>" alt="编辑" /></a>
						<a href="javascript:void(0)" onclick="delModel({link:'<?php echo IUrl::creatUrl("/order/ship_info_del/id/".$item['id']."");?>'})"><img class="operator" src="<?php echo $this->getWebSkinPath()."images/admin/icon_del.gif";?>" alt="删除" /></a>
					</td>
				</tr>
				<?php }?>
			</tbody>

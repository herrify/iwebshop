	...
	<div class="content form_content">
		<form action="<?php echo IUrl::creatUrl("/system/save_conf/form_index/base_conf");?>"  method="post" enctype='multipart/form-data' name='base_conf'>
			<table class="form_table">
				...
				<tr>
					<th>网站LOGO：</th>
					<td>
						<?php if($siteConfigObj->logo){?>
						<img src="<?php echo IUrl::creatUrl("")."".$siteConfigObj->logo."";?>" style="width:220px;" />
						<?php }?>
		...
		<form action='<?php echo IUrl::creatUrl("/system/save_conf/form_index/other_conf");?>' method='post' name='other_conf'>
			<table class="form_table">
				...
				<tr>
					<th>发货后订单自动完成时间：</th>
					<td><input type='text' class='small' name='order_finish_time' pattern='int' empty />天 <label>发货后的X天，会自动变为完成状态</label></td>
				</tr>
				<tr>
					<th>未付款订单自动取消时间：</th>
					<td><input type='text' class='small' name='order_cancel_time' pattern='int' empty />天 <label>未付款订单生成X天，会自动被取消掉</label></td>
				</tr>
				<tr>
					<th>发票税率：</th>

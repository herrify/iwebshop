...
<div class="headbar">
	...
	<form name="searchSelect" method="get" action="<?php echo IUrl::creatUrl("/");?>">
		...
		<input type='hidden' name='action' value='comment_list' />
		<div class="searchbar">
			购买时间：<input class="small" type="text" name="search[c.time>=]" onfocus="WdatePicker()" />
			截止时间：<input class="small" type="text" name="search[c.time<=]" onfocus="WdatePicker()" />
			回复状态：<select name="search[c.recomment_time]"><option value="">不限</option><option value="=0">未回复</option><option value=">0">已回复</option></select>
	...
	<table class="list_table">
		...
		<tbody>
			...
			<tr>
				...
				<td>
					<a href="<?php echo IUrl::creatUrl("/comment/comment_edit/cid/".$item['id']."");?>"><img class="operator" src="<?php echo $this->getWebSkinPath()."images/admin/icon_check.gif";?>" alt="查看" /></a>
					<a href="javascript:void(0)" onclick="delModel({link:'<?php echo IUrl::creatUrl("/comment/comment_del/check/".$item['id']."");?>'})"><img class="operator" src="<?php echo $this->getWebSkinPath()."images/admin/icon_del.gif";?>" alt="删除" /></a>
				</td>
			</tr>
			<?php }?>
		</tbody>

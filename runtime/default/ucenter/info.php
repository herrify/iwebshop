		...
		<dl class="userinfo_box clearfix">
			...
			<dd>
				<table class="form_table" width="100%" cellpadding="0" cellspacing="0">
					...
					<tr>
						<th>会员等级：</th><td><?php $query = new IQuery("user_group");$query->where = "id = {$this->userRow['id']}";$items = $query->find(); foreach($items as $key => $item){?><?php echo isset($item['group_name'])?$item['group_name']:"";?><?php }?></td>
					</tr>
				</table>
			</dd>
		</dl>
...
$(function(){
	...
	<?php if(isset($area[0]) && isset($area[1]) && isset($area[2])){?>

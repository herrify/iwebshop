...
<div class="ucenter container">
    ...
    <table class="list_table m_10" width="100%" cellpadding="0" cellspacing="0">
        ...
        <?php $member_info = Api::run('getMemberInfo',$user_id)?>
        <?php if($member_info['prop']!=''){?>
        <?php $ids=trim($member_info['prop'],',')?>
        <?php }?>
        <?php if(isset($ids) && $ids!=''){?>
        <tbody>
        <?php $page=(isset($_GET['page'])&&(intval($_GET['page'])>0))?intval($_GET['page']):1;?>
        <?php $queryPropList = Api::run('getPropList',$ids)?>
		<?php foreach($queryPropList->find() as $key => $item){?>
            <tr>
                <td><?php echo isset($item['card_name'])?$item['card_name']:"";?></td><td><?php echo isset($item['value'])?$item['value']:"";?>元</td><td><?php echo substr($item['end_time'],0,10);?></td><td class="blue"><?php if($item['is_userd']==1){?><span class="bold orange">已使用</span><?php }elseif($item['is_close']==1){?><span class="bold gray">已禁用</span><?php }elseif($item['is_close']==2){?><span class="bold gray">临时锁定</span><?php }elseif((strtotime('now')-strtotime($item['end_time']))>0){?><span class="bold light_gray">已过期</span><?php }else{?><span class="bold green">可使用</span><?php }?></td>
            </tr>
        <?php }?>
        </tbody>
        <tfoot><tr><td colspan="4" class="t_l"><?php echo $queryPropList->getPageBar();?></td></tr></tfoot>
        <?php }?>
    </table>
</div>

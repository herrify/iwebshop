...
<div class="ucenter container">
    ...
    <table class="list_table m_10" width="100%" cellpadding="0" cellspacing="0">
		...
		<?php foreach($querySuggestionList->find() as $key => $item){?>
            <tr>
                <td class="t_l"><?php echo isset($item['title'])?$item['title']:"";?></td><td><?php echo isset($item['time'])?$item['time']:"";?></td><td><?php if($item['re_content']!=''){?><span class="green bold">已回复</span><?php }else{?><span>未回复</span><?php }?></td><td><a class="blue bold" href="javascript:void(0)" onclick="show_msg(this,'<?php echo isset($item['title'])?$item['title']:"";?>','<?php echo isset($item['content'])?$item['content']:"";?>','<?php echo isset($item['admin'])?$item['admin']:"";?>','<?php echo isset($item['re_time'])?$item['re_time']:"";?>','<?php echo isset($item['re_content'])?$item['re_content']:"";?>')">查看</a></td>
            </tr>
        <?php }?>
        <tr id='show_msg' class="show" style="display:none">
            <td colspan="4">
				...
				<span id='re_info'>
				<hr class="dashed" />
				<b class="orange" id='admin'><?php echo $siteConfig->name;?></b> 在 [<span class="gray" id='re_time'></span>] 回复:<p class="gray indent mt_10" id='re_content'></p>
				</span>
			</td>
        </tr>
        </tbody>
        <tfoot><tr><td colspan="3" class="t_l"><?php echo $querySuggestionList->getPageBar();?></td></tr></tfoot>
    </table>
    <div class="orange_box">
		<form action="<?php echo IUrl::creatUrl("/ucenter/complain_edit");?>" method="post" >

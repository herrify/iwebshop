...
<div class="ucenter container">
	...
	<div class="uc_title m_10">
		<label><span><a href="<?php echo IUrl::creatUrl("/ucenter/evaluation");?>">未评价</a></span></label><label class="current"><span>已评价</span></label>
	</div>
	<table class="list_table m_10" width="100%" cellpadding="0" cellspacing="0">
		...
		<thead><tr><th>商品名称</th><th>订单编号</th><th>评论时间</th><th>评分</th><th>评价</th></tr></thead>
		<tbody>
			<?php $page= (isset($_GET['page'])&&(intval($_GET['page'])>0))?intval($_GET['page']):1;?>
			<?php $user_id = $this->user['user_id']?>
			<?php $queryIevaluationList = Api::run('getUcenterEvaluation',$user_id,1)?>
			<?php foreach($queryIevaluationList->find() as $key => $item){?>
			<tr>
				<td><a href="<?php echo IUrl::creatUrl("/site/comments_list/id/".$item['goods_id']."");?>" target="_block"><?php echo isset($item['name'])?$item['name']:"";?></a></td>
				<td><?php echo isset($item['order_no'])?$item['order_no']:"";?></td>
				<td><?php echo isset($item['comment_time'])?$item['comment_time']:"";?></td>
				<td><?php echo isset($item['point'])?$item['point']:"";?></td>
				<td><span class="green bold">已评价</span></td>
			</tr>
			<?php }?>
		</tbody>
		<tfoot><tr><td colspan="4" class="t_l"><?php echo $queryIevaluationList->getPageBar();?></td></tr></tfoot>
	</table>
</div>
